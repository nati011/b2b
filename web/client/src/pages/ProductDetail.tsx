...
const ProductDetail = () => {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-semibold text-primary mb-4">Product Not Found</h1>
          <Link to="/">
  ...
  const handleAddToCart = () => {
    const needsSize = 'sizes' in product;
    const needsColor = 'colors' in product;

    if ((needsSize && !selectedSize) || (needsColor && !selectedColor)) {
      toast.error(`Please select ${!selectedSize && needsSize ? 'size' : ''}${(!selectedSize && needsSize) && (!selectedColor && needsColor) ? ' and ' : ''}${!selectedColor && needsColor ? 'color' : ''}`);
      return;
    }
      ...
      <main className="container mx-auto px-4 pt-24 pb-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-8">
              <h3 className="text-sm font-medium text-primary mb-4">Quantity</h3>
