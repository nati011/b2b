    ...
    if (error) {
        return (
            <div className="w-full flex flex-col gap-6 items-center justify-center text-center min-h-screen">
                ...
                <Button size="lg" className="px-8" onClick={() => { fetchOrders(0, selectedStatus) }}>
                    Retry
                </Button>
            </div>
        );
                ...
                {orders.length === 0 ? (
                    ...
                    <Tabs defaultValue="ALL" onValueChange={setSelectedStatus}>
                        <TabsList className="w-full">
                            <TabsTrigger value="ALL" className="border-none">ALL</TabsTrigger>
                            <TabsTrigger value={PENDING_STATUS}>{PENDING_STATUS}</TabsTrigger>
                            <TabsTrigger value={COMPLETED_STATUS}>{COMPLETED_STATUS}</TabsTrigger>
                            <TabsTrigger value={CANCELED_STATUS}>{CANCELED_STATUS}</TabsTrigger>
                        </TabsList>
                        ...
                        <div className="space-y-6">
                            {
                                loading ? (
                                    ...
                                    <>
                                        {orders.map((order, i) => (
                                            <OrderCard key={i} completePayment={completePayment} loading={loading} order={order} />
                                        ))}
                                    </>
                                )
                            }
                            <Pagination>
                                <PaginationContent>
                                    ...
                                    {[...Array(Math.ceil(totalOrder / 10))].map((v, i) => (
                                        <PaginationItem onClick={() => setCurrentPage(i)} className="hover:cursor-pointer">
