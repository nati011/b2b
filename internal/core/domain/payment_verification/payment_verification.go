...
var (
	...
	ErrCannotProceedWithPaymentPartner = errors.New(" cannot proceed with payment partner")
	ErrCannotConfirmDigitalPayment     = errors.New(" cannot cofirm digital payment")
)
...
func (p *PaymentVerificationService) Callback(ctx context.Context, gatewayId int, txRef string) {
	...
	if is_verified {
		...
		_, err = p.order.UpdateConfirmationStatus(ctx, payment.OrderId, order.ORDER_CONFIRMED)
		if err != nil {
			log.Printf("Error Occured while updating payment status: %v", err)
		}
	}
