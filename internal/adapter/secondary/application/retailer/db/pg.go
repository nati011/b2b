...
func (p *Postgres) Create(ctx context.Context, req *port.CreateRequest) (int, error) {
	var resourceId int
	query := "SELECT * FROM public.create_retailer_user($1, $2, $3, $4, $5, $6);"
...
func (p *Postgres) UpdateBusiness(ctx context.Context, req *port.UpdateBusinessRequest) (int, error) {
	var resourceId int
	query := "SELECT * FROM public.create_retailer_business_location($1, $2, $3, $4, $5, $6);"
...
func (p *Postgres) GetByRetailerId(ctx context.Context, retailer_id int) (port.GetBusinessResponse, error) {
	var response port.GetBusinessResponse

	query := "SELECT * FROM public.get_retailer_business($1);"

	err := p.db.QueryRowContext(ctx, query, retailer_id).Scan(&response)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
			return port.GetBusinessResponse{}, port.ErrSysNoRows
