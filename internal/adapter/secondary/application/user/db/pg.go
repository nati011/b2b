...
func (p *Postgres) GetByID(ctx context.Context, id int) (port.GetResponse, error) {
	...
	err := p.db.QueryRowContext(ctx, query, id).Scan(&response.Id, &response.FirstName, &response.Email, &response.Phone, &response.Username, &response.DOB, &response.IsActive, &response.ExternalId)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
