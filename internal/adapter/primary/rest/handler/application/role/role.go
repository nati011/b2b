...
func (ro *Role) GetHandler(w http.ResponseWriter, r *http.Request) {
	...
	if paramIdValue != "" {
		...
		util.OperationSuccessResponse(w, util.Envelope{"roles": allRolesMapper(resp)})
	}
}
...
func (ro *Role) GetAllResourcesHandler(w http.ResponseWriter, r *http.Request) {
	...
	if paramIdValue != "" {
		...
		util.WriteJSON(w, util.Envelope{"resources": allResourcesMapper(resp)}, http.StatusAccepted)
	} else {
		resp, err := ro.service.GetAll(r.Context())
		if err != nil {
			switch err {
