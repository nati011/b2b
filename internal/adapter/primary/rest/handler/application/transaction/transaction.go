...
func (p *Transaction) GetTransactionsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if paramIdValue != "" {
		...
		util.OperationSuccessResponse(w, util.Envelope{"transaction": transactionMapper(resp)})
		...
		util.OperationSuccessResponse(w, util.Envelope{"transactions": allTransactionMapper(resp)})
