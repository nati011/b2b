...
case "${1:-up}" in
  up)
    ...
    echo ""
    echo "Stack is up. Ports bound to 0.0.0.0 (use host IP or localhost):"
    echo "  Nginx:        http://<host-ip>:8080/  (API + pgadmin at /pgadmin/)"
    echo "  Backend:      http://<host-ip>:8090/"
    echo "  pgAdmin:      http://<host-ip>:5051/"
