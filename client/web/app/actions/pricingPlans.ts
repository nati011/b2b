...
export async function fetchPricingPlans() {
  try {
    ...
    if (error.code === 'ERR_NETWORK' || error.message === 'Network Error') {
      const networkError = new Error('Unable to fetch supplier pricing. Please check your internet connection and try again.');
    ...
    // Generic error
    const genericError = new Error(error?.message || 'Unable to fetch supplier pricing. Please try again later.');
