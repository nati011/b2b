...
export default function AdminSupplierPage() {
  ...
  const fetchSuppliers = async (page: number, search: string = "") => {
    try {
      ...
      const response = await axios.get(`/supplier?${params.toString()}`);
      ...
      if (data.items && Array.isArray(data.items)) {
        ...
        totalCount = data.body.total || suppliersList.length;
    ...
    <div>
      <div className="mb-4 sm:mb-8">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">Suppliers</h1>
        </div>
                ...
                <TableBody>
                  {suppliers.map((supplier) => (
                    <TableRow key={supplier.id}>
                      ...
                      <TableCell>{supplier.business_name || 'N/A'}</TableCell>
                      <TableCell>{supplier.support_email || 'N/A'}</TableCell>
                      <TableCell>{supplier.support_phone || 'N/A'}</TableCell>
