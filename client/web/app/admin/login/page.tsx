...
export default function AdminLoginPage() {
  ...
  // Prefetch admin routes on mount
  useEffect(() => {
          ...
          if (userRolesStr) {
            try {
              ...
              if (normalizedRoles.includes('admin') || normalizedRoles.includes('superadmin')) {
                ...
                router.push('/admin');
                return;
              } else if (normalizedRoles.includes('supplier')) {
                // Supplier should go to supplier portal
                toast.success("Login successful!");
                router.push('/supplier');
                return;
              } else {
                // User doesn't have admin role - show debug info
                console.error('User roles:', userRoles);
                setErrors((prev) => ({ ...prev, general: `Access denied. Admin privileges required. Current roles: ${userRoles.join(', ') || 'none'}` }));
                setTimeout(() => {
                  setErrors({});
                }, 5000);
                return;
          ...
          <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
            <div className="flex flex-col items-center gap-2 text-left">
              ...
              <p className="text-balance text-sm text-muted-foreground">
                Enter your admin or supplier credentials to access the admin panel
              </p>
            </div>
